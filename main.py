...
from pitch_melody import PitchMelody
...
# (Midi note, ticks)
midi_file_pitches, midi_file_durations = notes_extractor.extract_notes(midi_file_path, channel=0)

# (ticks, nats)
entropy_calculator = EntropyCalculator()
midi_file_time_bins, midi_file_entropy_values = entropy_calculator.calculate_entropy_with_window(midi_file_path)
...
print(f"BPM Entropy: {audio_file_bpm_entropy}")
...
pitch_melody = PitchMelody()
pitch_values, pitch_confidence, pitch_times, hop_size, audio = pitch_melody.extract_pitch_melody(audio_file_path, audio_file_features)

pitch_MIDI = Note_segmentation_and_converting_to_MIDI()
onsets, durations, notes = pitch_MIDI.extract_pitch_MIDI(pitch_values, audio, hop_size)
...
filtered_pitch_values = pitch_values[pitch_confidence > 0]
filtered_pitch_times = pitch_times[pitch_confidence > 0]
...
onsets, durations, notes = estd.PitchContourSegmentation(hopSize=128)(pitch_values, audio)
#print("MIDI notes:", notes) # Midi pitch number
#print("MIDI note onsets:", onsets)
#print("MIDI note durations:", durations)
...
axarr[0].scatter(pitch_times, pitch_values, s=1.5)
axarr[0].set_title('estimated pitch [Hz]')
axarr[1].scatter(pitch_times, pitch_confidence, s=1.5)
